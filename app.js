...
(function () {
  ...
  const MAX_RETRIES = 1; // nouvelles tentatives sur échec réseau/blocage transitoire (jamais sur une vraie absence) — volontairement bas : le second passage en fin d'analyse (SECOND_PASS_CONCURRENCY, plus patient) prend le relais, pas la peine de retenter deux fois de suite au même rythme
  ...
  const BREAKER_RECOVERY_STREAK = 10; // succès consécutifs avant de relâcher la restriction
  ...
  const circuitBreaker = (function () {
    ...
    return {
      ...
      recordBlocked() {
        push(true);
        recoveryStreak = 0;
        // Ne redéclenche une pause que lors du passage à l'état "throttled" —
        // pas à chaque signal de blocage supplémentaire pendant qu'on y est
        // déjà, sinon un blocage persistant (mais réel) empile des pauses de
        // BREAKER_COOLDOWN_MS les unes après les autres au lieu de se
        // contenter de tourner au ralenti une seule fois.
        if (!throttled && ring.length >= BREAKER_MIN_SAMPLE) {
  ...
  const IMAGE_WARMUP_DELAY_MS = 150; // reste sous la limite de débit recommandée par Scryfall (~10 req/s)

  async function fetchImageBatch(batch, attempt) {
    attempt = attempt || 0;
    try {
      ...
      if (res.status === 429) {
        // Débit dépassé : on patiente (en respectant Retry-After s'il est
        // fourni) puis on retente, sans jamais insister plus de 3 fois.
        if (attempt >= 3) return;
        const retryAfterHeader = parseFloat(res.headers.get("Retry-After"));
        const waitMs = (isNaN(retryAfterHeader) ? 1.5 * (attempt + 1) : retryAfterHeader) * 1000;
        await sleep(waitMs);
        return fetchImageBatch(batch, attempt + 1);
      }
      if (!res.ok) return;
      const json = await res.json();
  ...
  // Requêtes strictement séquentielles et espacées (pas de concurrence) :
  // Scryfall recommande de rester autour de 10 requêtes/seconde en continu,
  // et une rafale de requêtes en parallèle déclenchait des erreurs 429.
  async function warmUpImageCache(names) {
    ...
    for (let i = 0; i < toFetch.length; i += IMAGE_WARMUP_BATCH_SIZE) {
      const batch = toFetch.slice(i, i + IMAGE_WARMUP_BATCH_SIZE);
      await fetchImageBatch(batch);
      await sleep(IMAGE_WARMUP_DELAY_MS);
    }
  }
